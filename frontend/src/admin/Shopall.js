...
const Shopall = () => {
    const [newItem, setNewItem] = useState([]);
      ...
      useEffect(() => {
        fetch(`http://localhost:4000/shopall`,)
          .then((res) => res.json())
  ...
  return (
    <div>
<button style={{backgroundColor:"black",position:"absolute",right:"10px"}}><Link to='/uploadshop' style={{textDecoration:"none",color:"white"}}>Create</Link></button>
        ...
        <tbody>
          {newItem.map((item) => (
            <tr key={item._id}>
              ...
              <td>{item.ds}</td>
              <td>
                <Link to={`/shopupdate/${item._id}`} style={{padding:"5px"}}>
                  <button>Edit</button>
                </Link>
