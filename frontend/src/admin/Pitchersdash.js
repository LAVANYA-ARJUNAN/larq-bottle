...
const Pitchersdash = () => {
 const [newItem, setNewItem] = useState([]);
      ...
      useEffect(() => {
        fetch(`http://localhost:4000/pitcher`,)
          .then((res) => res.json())
          .then((data) => setNewItem(data));
      }, []);

      const deletePro = (id) => {
          fetch(`http://localhost:4000/onepitcher/${id}`, {
            method: "DELETE",
            headers: {
  ...
  return (
    <div>
<button style={{backgroundColor:"black",position:"absolute",right:"10px"}}><Link to='/uploadpit' style={{textDecoration:"none",color:"white"}}>Create</Link></button>
        ...
        <tbody>
          {newItem.map((item) => (
            <tr key={item._id}>
              ...
              <td>{item.ds}</td>
              <td>
                <Link to={`/pitupdate/${item._id}`} style={{padding:"5px"}}>
                  <button>Edit</button>
                </Link>
